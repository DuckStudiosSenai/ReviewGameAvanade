...
public class Player : MonoBehaviourPun
{
    ...
    public GameManager_CatchGame gameManager;
    ...
    void Update()
    {
        ...
        {
            GameManager_CatchGame.lives -= 20;
        }
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Food"))
        {
            ...
            GameManager_CatchGame.lives += 20;
            if (GameManager_CatchGame.lives >= 100)
            {
                GameManager_CatchGame.lives = 100;
            }

            if (collectSound != null)
            {
                audioSource.PlayOneShot(collectSound);
                RandomFood.foodEaten += 1;
            }
    ...
    private void Death()
    {
        ...
        pointsManager.AddPoints(PlayerPrefs.GetInt("UserId"), GameManager_CatchGame.score);

        FindAnyObjectByType<DeathScreen>().ShowDeathScreen();
    }
    ...
    {
        if (GameManager_CatchGame.lives <= 0 && isAlive)
        {
            isAlive = false;
            Death();
